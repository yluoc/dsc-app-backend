...
export async function POST(request) {
  try {
    ...
    const { tokenCollateralAddress, amountCollateral, amountDscToBurn, privateKey } = body;

    // Validate required parameters
    const validationError = validateRequiredFields(body, ['tokenCollateralAddress', 'amountCollateral', 'amountDscToBurn', 'privateKey']);
    if (validationError) {
    ...
    if (!validateAmount(amountDscToBurn)) {
      return NextResponse.json(
    ...
    const result = await dscService.dscEngine.redeemCollateralForDSC(formattedToken, amountCollateral, amountDscToBurn);
    ...
    console.error('Error redeeming collateral and burning DSC:', error);
    return NextResponse.json(
      {
        success: false,
        error: 'Failed to redeem collateral and burn DSC',
